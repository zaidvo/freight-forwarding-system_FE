...
const companyOptions = [
  ...
  { id: "air-cargo", name: "Air Cargo", accent: "bg-sky-500" },
  { id: "road-network", name: "Road Network", accent: "bg-amber-500" },
];
      ...
      <div className="flex min-h-[calc(100vh-1rem)] overflow-hidden border border-slate-200/80 bg-white/60 shadow-[0_12px_44px_rgba(22,31,54,0.08)] backdrop-blur-xl lg:min-h-screen lg:border-x lg:border-y-0">
        ...
        >
          <div className="flex h-[68px] items-center gap-3 border-b border-slate-200/80 px-4">
            <div className="grid h-8 w-8 place-items-center rounded-[10px] bg-blue-500 text-white shadow-[0_10px_20px_rgba(59,130,246,0.24)]">
          ...
          <nav className="flex-1 space-y-3 overflow-y-auto px-2 py-3">
            {mainSections.map((section) => (
              <section key={section.title} className="space-y-1">
                {!collapsed && (
                  <div className="px-3 py-2 text-[11px] font-bold uppercase tracking-[0.12em] text-slate-400">
                    {section.title}
                  </div>
                )}
                {section.items.map((item) => {
                  ...
                  const content = (
                    <div
                      className={`group flex items-center gap-3 rounded-[12px] px-3 py-2.5 text-[14px] transition ${active ? "bg-blue-50 text-blue-600" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}
                    >
          ...
          <div className="border-t border-slate-200/80 p-3">
            {!collapsed && (
              <div className="relative mb-3">
                ...
                >
                  <div
                    className={`grid h-7 w-7 place-items-center rounded-lg text-white ${company.accent}`}
                  >
                ...
                {companyOpen && (
                  <div className="absolute inset-x-0 bottom-[62px] rounded-[14px] border border-slate-200 bg-white p-2 shadow-[0_18px_40px_rgba(22,31,54,0.12)]">
                    {companyOptions.map((option) => (
                      <button
                        key={option.id}
                        type="button"
                        onClick={() => setCompanyOpen(false)}
                        className={`flex w-full items-center gap-3 rounded-[12px] px-3 py-2.5 text-left text-[13px] transition hover:bg-slate-50 ${option.id === company.id ? "bg-blue-50" : ""}`}
                      >
                        <span
                          className={`h-2.5 w-2.5 rounded-full ${option.accent}`}
                        />
            ...
            {!collapsed && (
              <div className="flex items-center gap-3 rounded-[12px] px-2 py-2">
                <div className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-blue-100 to-blue-200 text-sm font-bold text-blue-600">
                  AM
                </div>
                <div className="min-w-0">
                  <div className="truncate text-[13px] font-semibold text-slate-900">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="flex h-[68px] items-center gap-4 border-b border-slate-200/80 bg-white/85 px-6 backdrop-blur-xl">
            <div className="text-[14px] text-slate-400">
              FreightOS <span className="mx-2 text-slate-300">/</span>{" "}
