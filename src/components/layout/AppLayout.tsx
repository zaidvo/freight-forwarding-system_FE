...
// Reads activeCompany from CompanyProvider → renders matching nav sections.
// Company switcher updates CompanyProvider which re-renders nav + dashboard.
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_right,_rgba(79,124,255,0.08),_transparent_28%),linear-gradient(180deg,#fbfcff_0%,#f4f6fb_100%)] px-2 py-2 text-slate-900 lg:px-0 lg:py-0">
      <div className="flex min-h-[calc(100vh-1rem)] overflow-hidden border border-slate-200/80 bg-white/60 shadow-[0_12px_44px_rgba(22,31,54,0.08)] backdrop-blur-xl lg:min-h-screen lg:border-x lg:border-y-0">
        {/* ── SIDEBAR ── */}
        <aside
          className={`flex shrink-0 flex-col border-r border-slate-200/80 bg-white/90 transition-[width] duration-200 ${
          ...
          <div className="border-t border-slate-200/80 p-3">
            ...
            {!collapsed && (
              <div className="relative mb-3">
                ...
                >
                  <div
                    className={`grid h-7 w-7 shrink-0 place-items-center rounded-lg text-white ${activeCompany.accent}`}
                  >
                  ...
                  >
                    {COMPANIES.map((option) => (
                      <button
                        ...
                        className={`flex w-full items-center gap-3 rounded-[12px] px-3 py-2.5 text-left text-[13px] transition hover:bg-slate-50 ${
                          option.id === activeCompany.id ? "bg-blue-50" : ""
                        }`}
                      >
                        <span
            ...
            {!collapsed && (
              <div className="flex items-center gap-3 rounded-[12px] px-2 py-2">
                ...
                <button
                  ...
                  className="grid h-7 w-7 shrink-0 place-items-center rounded-[8px] text-slate-400 transition hover:bg-rose-50 hover:text-rose-500"
                >
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="flex h-[68px] items-center gap-4 border-b border-slate-200/80 bg-white/85 px-6 backdrop-blur-xl">
            ...
            <div className="ml-auto flex items-center gap-3">
              ...
              >
                <span
                  className={`h-2.5 w-2.5 rounded-full ${activeCompany.accent}`}
                />
